...
description: Use when asked to "set up Vale", "add prose linting", "bootstrap the docs linter", "add a prose-lint CI job", "lint for AI tells", or "why is Vale so noisy?". Writes .vale.ini, seeds the vocabulary, adds the ai-tells style; never overwrites an existing config unprompted.
...
1. **Look before writing.** Check for an existing `.vale.ini` or `_vale.ini`.
   - **Present** → do **not** overwrite. Read it, report what it already covers, and offer a diff of what the reference config would add, including the `ai-tells` style if it is missing. Apply only with `--force` or explicit confirmation. A repo's own config always wins.
   - **Absent** → continue.
...
4. **Copy the `ai-tells` style.** It ships with the plugin, not as a downloadable package, so `vale sync` never fetches it (and leaves it in place):
   ...
   ```bash
   mkdir -p styles/ai-tells
   cp "<plugin-root>/references/vale-styles/ai-tells/"*.yml styles/ai-tells/
   ```

   The reference config already lists it in `BasedOnStyles`; a style named there with no directory is a hard error on the next lint (`E100 [loadStyles]`, exit 2). What each rule flags and why the dash rule is only a suggestion: `references/ai-tells.md`.

5. **Gitignore the downloaded styles.** `StylesPath = styles` holds downloadable packages, not source:
   ...
   The negations keep the vocabulary and the `ai-tells` style tracked, because both *are* source.

6. **Sync and run.** Report the real output; never claim a clean run that was not observed.
...
State: whether the config was written or skipped and why, whether the vocabulary directory was seeded, whether the `ai-tells` style was copied, whether `styles/` was gitignored, whether Vale is installed, the **actual** first-run alert counts by severity, and the recommended triage order.
